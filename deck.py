...
class Card():
    ...
    def __init__(self, suit: Literal["h", "s", "c", "d", "x"], value: Literal[3, 4, 5, 6, 7, 8, 9, 10, 21, 333, 777, 7734, 16], design : Literal["DEFAULT", "GALATIC", "LUXURY","OLD"] = "DEFAULT") -> None:
        self.suit: Literal["h", "s", "c", "d", "x"] = suit
        self.icon: Literal["♥️", "♠️", "♣️", "♦️", "🃏"] = "♥️" if suit == "h" else "♠️" if suit == "s" else "♣️" if suit == "c" else "♦️" if suit == "d" else "🃏"
...
'''
LAMAP_CARDS = [
    Card(h,3), Card(d,7), Card(d,6), Card(d,8),
    Card(d,4), Card(d,3), Card(h,8), Card(h,9), Card(h,10),
    Card(s,3), Card(c,3), Card(c,4), Card(c,5), Card(c,10)
]
'''

class Deck:
    """This class represents a shuffled deck of card"""
    def __init__(self, design: Literal["DEFAULT", "GALATIC", "LUXURY","OLD"] = "DEFAULT"):
        self.cards: list[Card] = [
            Card("h", 3, design), Card("h", 4, design), Card("h", 5, design),
            Card("h", 6, design), Card("h", 7, design), Card("h", 8, design),
            Card("h", 9, design), Card("h", 10, design),

            Card("c", 3, design), Card("c", 4, design), Card("c", 5, design),
            Card("c", 6, design), Card("c", 7, design), Card("c", 8, design),
            Card("c", 9, design), Card("c", 10, design),

            Card("d", 3, design), Card("d", 4, design), Card("d", 5, design),
            Card("d", 6, design), Card("d", 7, design), Card("d", 8, design),
            Card("d", 9, design), Card("d", 10, design),

            Card("s", 3, design), Card("s", 4, design), Card("s", 5, design),
            Card("s", 6, design), Card("s", 7, design), Card("s", 8, design),
            Card("s", 9, design),
        ].copy()
        self.design = design
